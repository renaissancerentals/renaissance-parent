...
public class MonitorService {
    ...
    private HealthMetadata buildErrorMetadata(MonitoringApp app,Exception e){
        ...
        return HealthMetadata.builder().status(HealthStatus.DOWN).updatedAt(now()).retryCounter(prev.retryCounter() + 1)
                .errorSince(errorSince)
                .errorMessage(Optional.ofNullable(e).map(Throwable::getMessage).orElse("Unknown"))
                .errorType(Optional.ofNullable(e).map(t -> t.getClass().getSimpleName()).orElse("Unknown"))
                .notificationSent(true).build();
    }

    private void updateAppHealth(MonitoringApp app,HealthMetadata metadata){
        appStore.compute(app.name(),(name,existingApp) -> {
            var updatedApp = MonitoringApp.builder().name(existingApp.name())
                    .healthEndpoint(existingApp.healthEndpoint()).health(metadata).build();
            ...
            } else if (metadata.status() == HealthStatus.UP && existingApp.health().status() == HealthStatus.DOWN) {
