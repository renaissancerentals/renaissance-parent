...
public class MonitorAppMapper {
    ...
    public MonitorErrorMessage mapToErrorMessage(MonitoringApp app) {
        return MonitorErrorMessage.builder()
                .applicationName(app.name())
                .status(app.health().status())
                .healthEndpoint(app.healthEndpoint())
                .retryCount(app.health().retryCounter())
                .time(ZonedDateTime.now(ZoneId.of(configProperties.monitoringZoneId()))
                        .format(LOCAL_DATE_TIME))
                .errorSince(app.health().errorSince().format(LOCAL_DATE_TIME))
                .errorMessage(app.health().errorMessage())
                .errorType(app.health().errorType())
                .build();
    }
    ...
    public MonitorHealthyMessage mapToHealthyMessage(MonitoringApp app, HealthMetadata previousHealth) {
        ...
        return MonitorHealthyMessage.builder()
                .applicationName(app.name())
                .previousStatus(previousHealth.status())
                .healthEndpoint(app.healthEndpoint())
                .time(now.format(LOCAL_DATE_TIME))
                .downtimeDuration(getDowntimeDuration(previousHealth.errorSince(), now))
                .build();
    }
