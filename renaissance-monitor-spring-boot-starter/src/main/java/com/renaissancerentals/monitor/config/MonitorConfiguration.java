...
public class MonitorConfiguration {
    @Bean
    public ApplicationRunner preloadApplications(MonitorConfigProperties configProperties,MonitoringAppStore appStore){
        return args -> {
            final var apps = configProperties.applications().entrySet().stream().collect(Collectors.toMap(
                    Map.Entry::getKey,
                    e -> MonitoringApp.builder().name(e.getKey()).healthEndpoint(e.getValue().healthEndpoint())
                            .health(HealthMetadata.builder().status(HealthStatus.UP)
