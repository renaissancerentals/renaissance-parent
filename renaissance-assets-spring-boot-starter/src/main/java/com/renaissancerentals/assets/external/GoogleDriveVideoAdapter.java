...
public class GoogleDriveVideoAdapter implements VideoService {
    ...
    public Optional<VideoAsset> getBy(String folderId,String name){
        try {
            ...
            Drive.Files.List list = drive.files().list()
                    .setQ(String.format("'%s' in parents and name = '%s'",folderId,name)).setPageSize(PAGE_SIZE)
                    .setOrderBy("modifiedTime desc").setFields("nextPageToken, files(" + VIDEO_FIELDS + ")");

            FileList result = list.execute();

            return result.getFiles().stream().filter(file -> file.getMimeType().contains("video"))
                    .map(this::buildVideoMetadata).findFirst();
    ...
    @Override
    public PagedResult<VideoAsset> listByFolder(final String folderId,final Integer pageSize,
            final String nextPageToken){
        try {
            log.debug("Listing drive: {} with pageSize: {} and pageToken {}",folderId,pageSize,nextPageToken);
            Drive.Files.List list = drive.files().list().setQ("'" + folderId + "' in parents")
                    .setPageSize(Optional.ofNullable(pageSize).orElse(PAGE_SIZE)).setOrderBy("modifiedTime desc")
            ...
            FileList result = list.execute();
            final var items = result.getFiles().stream().filter(file -> file.getMimeType().contains("video"))
                    .map(this::buildVideoMetadata).toList();

            return PagedResult.<VideoAsset>builder().pageSize(Optional.ofNullable(pageSize).orElse(PAGE_SIZE))
                    .items(items).nextPageToken(result.getNextPageToken()).build();
    ...
    private VideoAsset buildVideoMetadata(File file){
        return VideoAsset.builder().id(file.getId()).name(file.getName()).description(file.getDescription())
                .mimeType(file.getMimeType()).folderId(hasParents(file) ? file.getParents().getFirst() : null)
                .videoUrl(file.getWebContentLink()).thumbnailUrl(file.getThumbnailLink()).build();
    }
