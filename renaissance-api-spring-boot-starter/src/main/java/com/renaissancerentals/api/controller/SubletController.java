...
public class SubletController {
    ...
    public ResponseEntity<Sublet> get(@PathVariable("assetKey") String assetKey){
        ...
        return ResponseEntity.ok(subletRepository.getSublet(assetKey)
                .orElseThrow(() -> new NotFoundException(String.format("Asset Key %s not found",assetKey))));
    }

    @PostMapping("/{assetKey}/assets")
    public ResponseEntity<Asset> postAsset(@PathVariable("assetKey") String assetKey,
    ...
    public ResponseEntity<Void> sendMessage(@PathVariable("assetKey") String assetKey,
            @Valid @RequestBody SubletMessageRequest subletMessage){
