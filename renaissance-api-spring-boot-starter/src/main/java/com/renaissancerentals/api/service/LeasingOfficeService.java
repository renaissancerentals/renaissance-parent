...
public class LeasingOfficeService {
    ...
    public List<LeasingOfficeDetails> getAllLeasingOffices(){
        ...
        properties.forEach(property -> {
            if (property.getLeasingOfficeId() != null) {
                ...
                leasingOfficeProperties.get(property.getLeasingOfficeId())
                        .add(propertyMapper.toPropertyLeasingOffice(property));
            }
        });

        return leasingOffices.stream()
                .map(entity -> LeasingOfficeDetails.builder().id(entity.getId()).name(entity.getName())
                        .address(entity.getAddress()).zipcode(entity.getZipcode()).phone(entity.getPhone())
                        .officeHours(entity.getOfficeHours()).direction(entity.getDirection())
                        .officeMap(entity.getOfficeMap()).officeMapLandscape(entity.getOfficeMapLandscape())
                        .officeImage(entity.getOfficeImage()).officeImageDescription(entity.getOfficeImageDescription())
                        .properties(leasingOfficeProperties.getOrDefault(entity.getId(),List.of())).build())
                .toList();
    ...
    private CompletableFuture<List<LeasingOfficeEntity>> getLeasingOfficesAsync(){
        return CompletableFuture.supplyAsync(
                () -> StreamSupport.stream(leasingOfficeDao.findAll().spliterator(),false).toList(),
                virtualThreadExecutor);
    }

    private CompletableFuture<List<PropertyEntity>> getPropertiesAsync(){
        return CompletableFuture.supplyAsync(
                () -> StreamSupport.stream(propertyDao.findAll().spliterator(),false).toList(),virtualThreadExecutor);
    }
}
