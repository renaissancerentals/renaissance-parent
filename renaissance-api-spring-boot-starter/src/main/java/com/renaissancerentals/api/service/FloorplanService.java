...
public class FloorplanService {
    ...
    public Floorplan getFloorplan(String floorplanId){
        ...
        var floorplanEntity = floorplanFuture.join()
        ...
        return Floorplan.builder().id(floorplanEntity.getId()).name(floorplanEntity.getName())
                .bedroom(floorplanEntity.getBedroom()).bathroom(floorplanEntity.getBathroom())
                .style(floorplanEntity.getStyle()).allowedPet(floorplanEntity.getAllowedPet())
                .petPolicy(floorplanEntity.getPetPolicy()).featured(floorplanEntity.getFeatured())
                .featuredOnMain(floorplanEntity.getFeaturedOnMain()).patioIncluded(floorplanEntity.getPatioIncluded())
                .greenCertified(floorplanEntity.getGreenCertified()).address(floorplanEntity.getAddress())
                .zipcode(floorplanEntity.getZipcode()).videoTourLink(floorplanEntity.getVideoTourLink())
                .threeSixtyVideoTourLink(floorplanEntity.getThreeSixtyVideoTourLink())
                .virtualTourLink(floorplanEntity.getVirtualTourLink()).photo(floorplanEntity.getPhoto())
                .coverImage(floorplanEntity.getCoverImage()).floorPlanFolderId(floorplanEntity.getFloorPlanFolderId())
                .photosFolderId(floorplanEntity.getPhotosFolderId()).photosCount(floorplanEntity.getPhotosCount())
                .description(floorplanEntity.getDescription()).vanityLink(floorplanEntity.getVanityLink())
                .htmlTitle(floorplanEntity.getHtmlTitle()).metaDescription(floorplanEntity.getMetaDescription())
                ...
                .customCode(floorplanEntity.getCustomCode()).highlights(floorplanEntity.getHighlights())
                ...
                .property(propertyFuture.join().orElseThrow(
                        () -> new NotFoundException(String.format("Property for floorplan %s not found",floorplanId))))
                .units(unitsFuture.join()).webSpecials(webSpecialsFuture.join()).amenities(amenitiesFuture.join())
                .utilities(utilitiesFuture.join()).build();
    ...
    public List<SimilarFloorplan> findSimilarFloorplans(String floorplanId){
        return similarFloorplanDao.findAllByFloorplanId(floorplanId).stream()
                .map(entity -> new SimilarFloorplan(entity.getSimilarFloorplanId())).toList();
    }
    ...
    public List<FloorplanVariation> findFloorplanVariations(String floorplanId){
        return floorplanVariationDao.findAllByFloorplanId(floorplanId).stream()
                .map(entity -> new FloorplanVariation(entity.getVariation())).toList();
    }

    public List<Testimonial> findFloorplanTestimonials(String floorplanId){
        return testimonialDao.findAllByFloorplanId(floorplanId).stream().map(
                entity -> Testimonial.builder().tenant(entity.getTenant()).testimonial(entity.getTestimonial()).build())
                .toList();
    }
    ...
    public List<Faq> findFloorplanFaqs(String floorplanId){
        return floorplanFaqDao.findAllByFloorplanId(floorplanId).stream().map(faqMapper::toFaq).toList();
    }

    private CompletableFuture<Optional<FloorplanEntity>> getFloorplanAsync(String floorplanId){
        return CompletableFuture.supplyAsync(() -> floorplanDao.findById(floorplanId),virtualThreadExecutor);
    }
}
