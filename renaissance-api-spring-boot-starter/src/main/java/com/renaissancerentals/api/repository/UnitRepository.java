...
public class UnitRepository {
    ...
    public UnitUtilities getUnitUtilities(String unitId) {
        ...
        var floorplan = floorplanRepository
                .findById(unit.getFloorplanId())
                .orElseThrow(() -> new NotFoundException("Floorplan not found"));

        var property = propertyRepository
                .findById(floorplan.getPropertyId())
                .orElseThrow(() -> new NotFoundException("Property not found"));

        var utilities = utilityRepository.findByFloorplanId(floorplan.getId()).stream()
                .map(utilityEntity -> Utility.builder()
                        .id(utilityEntity.getId())
                        .name(utilityEntity.getName())
                        .type(utilityEntity.getType())
                        .averageMonthlyBill(utilityEntity.getAverageMonthlyBill())
                        .build())
                .toList();
        return UnitUtilities.builder()
                .id(unit.getId())
                .utilities(utilities)
