...
public class SubletRepository {
    ...
    public List<Sublet> getAll(){
        return subletDao.findByActiveTrueAndApprovedTrue().stream().map(subletMapper::fromEntity).toList();
    }
    ...
    public Asset createAsset(String assetKey,MultipartFile file,String name,Boolean isCoverImage){
        var sublet = subletDao.findOneByAssetKey(assetKey)
                .orElseThrow(() -> new NotFoundException("Sublet not found!"));
        if (isCoverImage) {
            var asset = assetService.create(AssetCreationRequest.builder().folderId(sublet.getSubletFolderId())
                    .multipartFile(file).name(name).build());
            sublet.setCoverImage(asset.original());
            subletDao.save(sublet);
            return asset;
        } else {
            return assetService.create(AssetCreationRequest.builder().folderId(sublet.getPhotosFolderId())
                    .multipartFile(file).name(name).build());
        }
    }
