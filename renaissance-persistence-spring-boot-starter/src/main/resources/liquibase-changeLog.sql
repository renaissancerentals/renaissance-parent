...
-- changeset renaissance-admin:78
...
(
    ...
    from_phone      TEXT,
    source_url      TEXT,
    phone_preferred boolean              DEFAULT false,
    text_preferred  boolean              DEFAULT false,
    email_preferred boolean              DEFAULT false,
    raw_question    TEXT,
    additional_info JSONB,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

--changeset renaissance-admin:79

CREATE INDEX idx_property_listing
    ON property (lease_type, id) WHERE active = true;

CREATE INDEX idx_floorplan_listing
    ON floorplan (property_id) WHERE active = true
  AND style != 'GARAGE';

CREATE INDEX idx_unit_listing
    ON unit (floorplan_id) WHERE active = true;

--changeset renaissance-admin:80
ALTER TABLE unit
    add column floorplan_image varchar(255);

-- changeset renaissance-admin:81

ALTER TABLE application_email
...
-- changeset renaissance-admin:82
UPDATE application_email
SET first_name = split_part(name, ' ', 1),
    last_name  = CASE
                     WHEN name LIKE '% %' THEN substring(name from ' ([^ ]+)$')
                     ELSE ''
        END;
...
-- changeset renaissance-admin:83
ALTER TABLE application_email
DROP
COLUMN name;
...
-- changeset renaissance-admin:84
ALTER TABLE home_page_special
DROP COLUMN IF EXISTS title,
    DROP COLUMN IF EXISTS details,
    DROP COLUMN IF EXISTS information1,
    DROP COLUMN IF EXISTS information2,
    DROP COLUMN IF EXISTS information3,
    ADD COLUMN IF NOT EXISTS links JSONB;

-- changeset renaissance-admin:85
