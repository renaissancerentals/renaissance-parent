...
public class ExceptionNotifier<T extends BaseException> {
    ...
    public void notifyException(final T notifyingException) {
        ...
        if (isExcluded(notifyingException)) return;

        if (!rateLimiter.shouldTrigger(notifyingException.getClass().getName())) return;
        ...
        final var title = MessageFormat.format(
                properties.titleFormat(), notifyingException.getErrorMessage().application());

        var mailMessage = MailMessage.builder()
                .subject(title)
                .to(properties.emailTo())
                .cc(properties.emailCc())
                .build();
    ...
    private boolean isExcluded(final T notifyingException) {
        return properties
                .excludedServerExceptions()
                .contains(notifyingException.getClass().getName());
    }

    private ServerErrorMessage messageFrom(final T notifyingException) {
        if (notifyingException == null) {
            return ServerErrorMessage.builder()
                    .message("Unknown error")
                    .timeStamp(Instant.now().toString())
                    .sourceName("UnknownException")
                    .exceptionClass("java.lang.NullPointerException")
                    .errorCode("UNKNOWN")
                    .stackTrace("Exception was null")
                    .build();
        }
        ...
        return ServerErrorMessage.builder()
                ...
                .timeStamp(Instant.now().toString())
                .sourceName(notifyingException.getClass().getSimpleName())
                .exceptionClass(notifyingException.getClass().getName())
                .errorCode(errorCode != null ? errorCode : "UNKNOWN")
                .stackTrace(Arrays.toString(notifyingException.getStackTrace()))
                .build();
    }
}
